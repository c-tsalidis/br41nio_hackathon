            ...
            if (numberOfBytesReceived > 0) {
                ...
                for (int i = 56; i < 63; i++) {
                    if(baselineWaitingTimeFinished) {
                        ...
                        var neuronRate = GetMappedDifferences(0.2f, 1)[2];// alpha
                        var temperature = GetMappedDifferences(3000, 8000)[2]; // alpha
                        var spawnDistance = GetMappedDifferences(50, 300)[2]; // alpha
                        var cameraSpeed = GetMappedDifferences(0.01f, 0.03f)[2]; // alpha
                        // VisualManager.SetParameters(Mathf.Abs( 1 - neuronRate), Mathf.Abs( 8000 - temperature), Mathf.Abs( 300 - spawnDistance), Mathf.Abs( 0.03f - cameraSpeed));
                        print(neuronRate + " - " + temperature  + " - " +  spawnDistance  + " - " +  cameraSpeed);
                        VisualManager.SetParameters(neuronRate, temperature, spawnDistance, cameraSpeed);
